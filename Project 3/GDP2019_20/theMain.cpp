...
// View matrix
		...
		glm::vec3 cameraEye = glm::vec3(player1->getPositionXYZ().x + cameraLeftRight, player1->getPositionXYZ().y + 20.0f, player1->getPositionXYZ().z - 50.0f);
		v = glm::lookAt(cameraEye,
			player1->getPositionXYZ(),
			upVector);
...
//
		...
		for (int index = 0; index != ::g_vec_pPlayerObjects.size(); index++)
		{
			...
			iObject* pCurrentObject = ::g_vec_pPlayerObjects[index];
		...
		{
			player1->setVelocity(player1->getVelocity() * glm::vec3(-1.0f, -1.0f, -1.0f));
			player1->setVelocity(player1->getVelocity() * glm::vec3(0.95f, 1.0f, 1.0f));
			player1->setVelocity(player1->getVelocity()* glm::vec3(-1.0f, -1.0f, -1.0f));
		}
		if (player1->getVelocity().z < 0)
		{
			player1->setVelocity(player1->getVelocity()* glm::vec3(-1.0f, -1.0f, -1.0f));
			player1->setVelocity(player1->getVelocity() * glm::vec3(1.0f, 1.0f, 0.95f));
			player1->setVelocity(player1->getVelocity()* glm::vec3(-1.0f, -1.0f, -1.0f));
		}
		...
		if (player1->getVelocity().x > 0)
		{
			player1->setVelocity(player1->getVelocity() * slowX);
		}
		if (player1->getVelocity().z > 0)
		{
			player1->setVelocity(player1->getVelocity()* slowZ);
		}
		...
		for (int k = 0; k < ::g_vec_pWallObjects.size(); k++)
		{
			...
			pPhsyics->GetClosestTriangleToPoint(player1->getPositionXYZ(), transformedMesh, closestPoint, closestTriangle);
			...
			float distance = glm::length(player1->getPositionXYZ() - closestPoint);

			if (distance <= player1->get_SPHERE_radius())
			{
				//if (k == 0)
				//{
				//	player1->inverseMass = 0.0f;
				//	player1->positionXYZ = glm::vec3(0.0f, 0.0f, 0.0f);
				//}
				...
				glm::vec3 vecSphereToClosestPoint = closestPoint - player1->getPositionXYZ();
				...
				float lengthPositionAdjustment = player1->get_SPHERE_radius() - centreToContractDistance;

				// 4. Sphere is moving in the direction of the velocity, so 
				//    we want to move the sphere BACK along this velocity vector
				glm::vec3 vecDirection = glm::normalize(player1->getVelocity());

				glm::vec3 vecPositionAdjust = (-vecDirection) * lengthPositionAdjustment;

				// 5. Reposition sphere 
				player1->setPositionXYZ(player1->getPositionXYZ() + vecPositionAdjust);
				...
				glm::vec3 velocityVector = glm::normalize(player1->getVelocity());
				float gravY = (-player1->getVelocity().y) * 0.45f;
	...
	//			player1->inverseMass = 0.0f;	// Stopped
				...
				float speed = glm::length(player1->getVelocity());



				if (player1->getVelocity().y * -1 < 1 && !onGround)
				{
					onGround = true;
					properYPosition = player1->getPositionXYZ().y;
				}
				if (!onGround)
				{
				...
				{
					player1->setVelocity(reflectionVec * speed);
					glm::vec3 properPositionXYZ = glm::vec3(player1->getPositionXYZ().x, properYPosition, player1->getPositionXYZ().z);
					player1->setPositionXYZ(properPositionXYZ);
				}
			...
			glm::vec3 CentreToClosestPoint = player1->getPositionXYZ() - closestPoint;

			// Direction that ball is going is normalized velocity
			glm::vec3 directionBall = glm::normalize(player1->getVelocity());	// 1.0f
			...
			pDebugRenderer->addLine(player1->getPositionXYZ(),
				closestPoint,
				glm::vec3(0.0f, 1.0f, 0.0f),
				1.0f);
		...
		for (int k = 0; k < ::g_vec_pRampObjects.size(); k++)
		{
			...
			if (distance <= player1->get_SPHERE_radius())
			{
				...
				glm::vec3 velocityVector = glm::normalize(player1->getVelocity());
				float gravY = (-player1->getVelocity().y) * 0.25f;
				glm::vec3 gravity = glm::vec3(0.0f, gravY, 0.0f);
